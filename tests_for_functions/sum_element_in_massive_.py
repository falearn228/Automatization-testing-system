...
def test_1():
    return [a := np.random.randint(0, 100, size := randint(4, 20)).tolist(), size, sum(a)]
...
def test_3():
    return [a := np.random.randint(-100, -1, size := randint(4, 20)).tolist(), size, sum(a)]


def test_4():
    return [a := np.random.randint(0, 100, size := 1).tolist(), size, sum(a)]


def test_5():
    return [a := np.random.randint(0, 100, size := randint(100, 200)).tolist(), size, sum(a)]


#
def test_6():
    return [a := np.zeros((size := randint(4, 20)), dtype=int).tolist(), size, sum(a)]
...
def test_7():
    ...
    size = size1 + size2 + 1
    a = np.union1d(a, d)
    return [c := np.union1d(a, b).tolist(), len(c), sum(c)]
...
with open(r"C:\Users\falearn\source\repos\the_func\sum_element_in_massive.json", "w") as outfile:
